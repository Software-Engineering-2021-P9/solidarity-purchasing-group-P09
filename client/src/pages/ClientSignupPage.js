...
import { createClient, createClientAccount } from "../services/ApiClient";
...
function ClientSignupPage(props) {
  ...
  return (
    <Container>
      <Row>
        <NavbarComponent links={getAvailableNavbarLinks("")} loggedUser={authContext.currentUser} />
      </Row>
      <Row>
...
function ClientForm(props) {
  ...
  const [client, setClient] = useState({ firstName: "", lastName: "",phoneNumber: "", email: " ",password:"",passwordConfirmation:"", address:"", number:"", city:"", postCode:"" });
  const cancelErrors=()=>{
    setErrorMessage("");
    setErrorMessage1("");
    setErrorMessage2("");
  ...
  const handleSubmit = (event) => {
    ...
    if(props.authContext.currentUser === null ){
      if(client.password.localeCompare(client.passwordConfirmation)){
        ...
        setErrorPassword("Mismatch passwords!");
      }
    }
          ...
          <><Row className="mb-3">
            <Form.Group as={Col} md="3" controlId="validationCustom02">
              <Row>
                ...
                <Col>
                  <Form.Control value ={client.password}  onChange={handleChange} name = "password" 
                  required
                   type="password" 
                    />
                </Col>
          ...
          <Row>
            <Form.Group as={Col} md="3" controlId="validationCustom02">
              <Row>
                ...
                <Col>
                  <Form.Control value ={client.passwordConfirmation}  onChange={handleChange} name = "passwordConfirmation" 
                  required
                   type="password" 
                    />
                ...
                {showErrorPassword ? (
            <Alert variant="danger" onClose={() => setShowErrorPassword(false)} dismissible>
              {errorPassword}
