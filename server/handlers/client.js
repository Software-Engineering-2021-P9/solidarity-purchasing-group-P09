...
exports.loginEmailValidatorChain = [emailBodyValidator];
exports.getClientByEmailPasswordHandler = async function (req, res) {
  console.log(req.body.email);
  console.log(req.body.password);
  let result;
  try {
    result = await dao.getClientByEmail(req.body.email.toString());
  } catch (err) {
    console.error(`getClientByEmailPasswordHandler() -> Credentials are wrong`);
    return res.status(404).end();
  }

  if (result) {
    bcrypt
      .compare(req.body.password.toString(), result.password)
      .then((isTrue) => {
        if (isTrue) {
          // password matches
        ...
        console.error(
          `getClientByEmailPasswordHandler() -> Credentials are wrong`
        );
        return res.status(404).end();
      });
  } else {
